...
class AdminController extends App_Controller
{
     public function __construct()
    {
        ...
        // 🔐 Restrict agents from accessing any /admin/* page
        $role = $this->session->userdata('role'); // stored at login
        if ($role === 'agent') {
            $this->session->set_flashdata('access_denied', 'Access Denied! Agents cannot access admin pages.');
