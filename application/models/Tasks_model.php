...
class Tasks_model extends App_Model
{
  ...
  public function sync_task_assignees($task_id, array $assignees)
{
    $task_id = (int)$task_id;

    $current = $this->db->select('staffid')
        ->from(db_prefix() . 'task_assigned')
        ->where('taskid', $task_id)
        ->get()->result_array();

    $currentIds = array_map(function ($r) {
        return (int)$r['staffid'];
    }, $current);

    $toAdd = array_values(array_diff($assignees, $currentIds));
    $toDel = array_values(array_diff($currentIds, $assignees));

    if (!empty($toDel)) {
        $this->db->where('taskid', $task_id);
        $this->db->where_in('staffid', $toDel);
