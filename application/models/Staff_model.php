...
class Staff_model extends App_Model
{
    ...
    {
        // Optional: You can apply hooks here if needed
        $this->db->select('*');
        $this->db->from(db_prefix() . 'staff t');
        $this->db->join(db_prefix() . 'roles r', 't.role = r.roleid');
        $this->db->where('r.name', 'Agent');
    ...
    public function get_employees_and_management()
    {
        $this->db->select('*');
        $this->db->from(db_prefix() . 'staff t');
        $this->db->join(db_prefix() . 'roles r', 't.role = r.roleid');
        
        // Exclude only "Agent" role (case-insensitive)
        $this->db->where('LOWER(r.name) !=', 'agent');
    ...
    public function get_staff_permissions($id)
    {
        ...
        return $permissions;
  	  	...
  	  	public function add_agent_from_register($post)
{
    $this->db->where('LOWER(name)', 'agent');
    ...
    $staffData = [
        ...
        'phonenumber'        => isset($post['phone']) ? $post['phone'] : '',
        'role'               => $role->roleid,
        'active'             => 1,
        'admin'              => 0,
        'is_not_staff'       => 0,
        'default_language'   => get_option('active_language'),
        'direction'          => get_option('rtl_support_admin') == 1 ? 'rtl' : 'ltr',
        'agent_id'           => str_pad($nextId, 7, '0', STR_PAD_LEFT),
        'send_welcome_email' => 1,
    ];

    return $this->add($staffData);
}
