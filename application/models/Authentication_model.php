    ...
    if ((!empty($email)) and (!empty($password))) {
        ...
        if ($user) {
            ...
            if (!$user->password || !app_hasher()->CheckPassword($password, $user->password)) {
                ...
                log_activity('Failed Login Attempt [Email: ' . $email . ', Is Staff Member: ' . ($staff ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
                return false;
            }
        } else {
            ...
            log_activity('Non Existing User Tried to Login [Email: ' . $email . ', Is Staff Member: ' . ($staff ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            return false;
        }
        ...
        if ($user->active == 0) {
            ...
            log_activity('Inactive User Tried to Login [Email: ' . $email . ', Is Staff Member: ' . ($staff ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            return [
                'memberinactive' => true,
            ];
        }
        ...
        if ($staff == true) {
            ...
            $user_data = [
              ...
              // 🚀 Preserve login_context before overwriting session
        $current_context = $this->session->userdata('login_context');
        ...
        // Restore context
        if ($current_context) {
            $this->session->set_userdata('login_context', $current_context);
        }
...
public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            $new_pass_key = app_generate_hash();
          	$context = $this->session->userdata('login_context') ?: ($staff ? 'admin' : 'reseller');
        	$reset_url = site_url($context . '/authentication/reset_password/' . $new_pass_key);
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $data['staff']        = $staff;
                $data['userid']       = $user->$_id;
              $data['reset_password_url'] = $reset_url;
                $merge_fields         = [];
