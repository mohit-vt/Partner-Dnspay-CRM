...
$current_user = $this->staff_model->get($staff_id);
$rolevalue = $this->roles_model->get($current_user->role);

// If current user's role name is 'Employee', allow only 'Agent' in the dropdown
if (isset($rolevalue->name) && $rolevalue->name === 'reseller') {
    $roles = array_filter($roles, function ($role) {
        return strtolower(is_array($role) ? $role['name'] : $role->name) === 'agent';
    });
}
?>
