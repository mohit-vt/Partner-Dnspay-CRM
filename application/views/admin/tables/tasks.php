...
return App_table::find('tasks')
    ->outputUsing(function ($params) {
        ...
        if ($hasPermissionEdit) {
            ...
            $allActiveStaff = $this->ci->staff_model->get('', ['active' => 1]);
        }
        ...
        <a href="#" class="' . $class . ' tasks-table-start-timer" onclick="timer_action(this,' . $aRow['id'] . '); return false;">' . _l('task_start_timer') . '</a>
            ...
            // Editable Assigned-to column
            if ($hasPermissionEdit && $aRow['status'] != Tasks_model::STATUS_COMPLETE) {

                $selectedIds = [];
                if (!empty($aRow['assignees_ids'])) {
                    $selectedIds = array_filter(array_map('intval', explode(',', $aRow['assignees_ids'])));
                }

                $assignedHtml = '<select class="task-assignees-inline selectpicker"
                                    multiple
                                    data-live-search="true"
                                    data-width="100%"
                                    data-taskid="' . (int)$aRow['id'] . '"
                                    title="' . _l('task_assigned') . '">';

                foreach ($allActiveStaff as $m) {
                    // Perfex staff_model usually returns staffid, firstname, lastname
                    $sid = (int)$m['staffid'];
                    $selected = in_array($sid, $selectedIds, true) ? ' selected' : '';
                    $assignedHtml .= '<option value="' . $sid . '"' . $selected . '>'
                        . e($m['firstname'] . ' ' . $m['lastname'])
                        . '</option>';
                }
