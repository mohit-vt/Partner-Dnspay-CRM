...
(function () {
  ...
  // Re-render selectpicker whenever datatable redraws (paging/filtering)
  $(document).on('draw.dt', '.table-tasks', function () {
    $('.task-assignees-inline').selectpicker('render');
  });

  // Save on change
  $(document).on('changed.bs.select', '.task-assignees-inline', function () {
    var $el = $(this);
    var taskId = $el.data('taskid');
    var assignees = $el.val() || [];

    $.post(admin_url + 'tasks/update_task_assignees_inline/' + taskId, {
      assignees: assignees
    }).done(function (res) {
      try { res = JSON.parse(res); } catch(e) {}

      if (res && res.success) {
        alert_float('success', 'Assigned updated');
      } else {
        alert_float('warning', 'Could not update assignee(s)');
      }
    }).fail(function () {
      alert_float('warning', 'Could not update assignee(s)');
    });
