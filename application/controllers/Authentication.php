  	...
  	public function agent_register()
{
    if ($this->input->post()) {
        ...
        $this->form_validation->set_rules(
            'email',
            'Email',
            'trim|required|valid_email|is_unique[' . db_prefix() . 'staff.email]'
        );
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $agent_id = $this->staff_model->add_agent_from_register($this->input->post());

            if ($agent_id) {
                set_alert('success', 'Agent account created successfully. You can now login.');
                redirect(admin_url('authentication'));
            } else {
                set_alert('danger', 'Agent registration failed. Please try again.');
            }
        }
    }

    $data['title'] = 'Agent Register';
    $data['bodyclass'] = 'register agent-register';

    $this->data($data);
    $this->view('agent_register');
    $this->layout();
}
