    ...
    public function wtl($key = '')
    {
        ...
        if ($this->input->post('key')) {
            if ($this->input->post('key') == $key) {
                ...
                $user_answer = $post_data['user_captcha'];
                $correct_answer = $post_data['captcha_answer'];
        
                // Check if the user answered correctly
                if ($user_answer != $correct_answer) {
                    echo json_encode([
                        'success' => false,
                        'message' => "Wrong Captch Answer",
                    ]);
                ...
                if ($insert_to_db == true) {
                    ...
                    $regular_fields['from_form_id'] = $form->id;
                  	$regular_fields['user_id'] 		= get_staff_user_id();
                    $regular_fields['is_public']    = $form->mark_public;
                    $regular_fields['remote_ip']    = $_SERVER['REMOTE_ADDR'];
                    $regular_fields['user_agent']   = get_browser_name($_SERVER['HTTP_USER_AGENT']);
                    $regular_fields['page_url']     = isset($_POST['page_url']) && !empty($_POST['page_url']) ? $_POST['page_url'] : 'No URL Captured';
                  
                 

                        // --- Generate Agent ID ---
                        $this->db->select_max('agent_id');
                        $lastAgent = $this->db->get(db_prefix().'leads')->row();
                        $nextAgentId = ($lastAgent && $lastAgent->agent_id) ? $lastAgent->agent_id + 1 : 360501;

                        // --- Generate Application ID ---
                        $monthLetter = strtoupper(substr(date('F'), 0, 1)); // S for September
                        $yearTwo     = date('y');                           // 25 for 2025

                        $this->db->select('application_id');
                        $this->db->like('application_id', $monthLetter . $yearTwo, 'after');
                        $this->db->order_by('id', 'DESC');
                        $lastApp = $this->db->get(db_prefix().'leads')->row();

                        if ($lastApp && preg_match('/^[A-Z][0-9]{2}([0-9]+)$/', $lastApp->application_id, $matches)) {
                            $sequence = (int)$matches[1] + 1;
                        } else {
                            $sequence = 3661; // start from 3661
                        }

                        $applicationId = $monthLetter . $yearTwo . $sequence;

                        // Save into insert fields
                        $regular_fields['agent_id']       = $nextAgentId;
                        $regular_fields['application_id'] = $applicationId;
