...
class Pre_application extends AdminController
{
    ...
    if (stripos(uri_string(), 'admin/pre_application') !== false) {
        show_error('❌ Access Denied: Direct access via /admin/pre_application is not allowed. Please use /reseller/pipeline_report.', 403, 'Forbidden');
        exit;
    }
  
  	   // 🔒 Hard block: If URL contains /admin/pipeline_report, deny access immediately
    if (stripos(uri_string(), 'admin/pre_application/create') !== false) {
        show_error('❌ Access Denied: Direct access via /admin/pre_application/create is not allowed. Please use /reseller/pipeline_report.', 403, 'Forbidden');
        exit;
    }
    ...
    if (is_admin()) {
        ...
        $this->load->view('reseller/pre_application/list', $data);
    }

  	 public function pre_application_detail() {
       
     $ref = $this->input->get('ref'); // capture ?ref= value

    if ($ref) {
        // Store in session or use for tracking
        $this->session->set_userdata('affiliate_ref', $ref);
    }
    ...
    if (is_admin()) {
        ...
        $this->load->view('reseller/pre_application/pre_application_detail', $data);
    ...
    public function create()
    {
        ...
        if ($this->input->post()) {
            ...
            // Attach logged in staff ID
            $formData['user_id'] = get_staff_user_id();

            $insertId = $this->pre_application_model->add_pre_application($formData);

            if ($insertId) {
                set_alert('success', 'Merchant Pre Application created successfully.');
            } else {
                set_alert('danger', 'Failed to create Merchant Pre Application.');
            }
        }

        $data['title'] = 'Create Merchant Pre Application';
        $this->load->view('reseller/pre_application/pre_application', $data);
    }
    ...
    public function view($id)
    {
        ...
        $this->load->view('reseller/pre_application/view', $data);
    }
    ...
    public function copy($id)
    {
        ...
        if (!$application) {
            ...
            redirect(site_url('reseller/pre_application'));
        }
...
public function update($id)
{
    ...
    // Fetch existing record for file merging
    $existing_record = $this->pre_application_model->get_by_id($id);
    ...
    if (!empty($_FILES['upload_file']['name'][0])) {
        ...
        $data['upload_file'] = $existing_record->upload_file; // retain old files
    }

    // Update record
    //$this->pre_application_model->update_pre_application($id, $data);
  
   $updated  = $this->pre_application_model->update_pre_application($id, $data);

     if ($updated) {
    	set_alert('success', 'Merchant Pre Application updated successfully.');
      redirect(site_url('reseller/pre_application/view/' . $id)); // Stay on same page
  } else {
      set_alert('danger', 'No changes made or update failed.');
      redirect(site_url('reseller/pre_application/view/' . $id)); // Still stay on same page
  }
    ...
    // ✅ Redirect to stay on the same record page
    redirect(site_url('reseller/pre_application/view/' . $id));
}
...
$uploaded_files = [];
if (!empty($_FILES['upload_file']['name'][0])) {
    $filesCount = count($_FILES['upload_file']['name']);
    for ($i = 0; $i < $filesCount; $i++) {
        ...
        $config['upload_path']   = './uploads/pre_applications/';
        $config['allowed_types'] = 'jpg|jpeg|png|gif|pdf';
        $config['max_size']      = 2048; // 2MB
        $config['encrypt_name']  = TRUE; // unique file names
...
// Save file names into DB (JSON format)
if (!empty($uploaded_files)) {
    $data['upload_file'] = json_encode($uploaded_files);  // match DB column name
}
            ...
            if (!empty($errors)) {
               ...
               redirect(site_url('reseller/pre_application/form'));
                return;
            }
        ...
        if ($insert_id) {
            set_alert('success', 'Pre-Application submitted successfully.');
            redirect(base_url('reseller/pre_application/create'));
        } else {
            set_alert('danger', 'Failed to submit Pre-Application.');
            redirect(base_url('reseller/pre_application/create'));
        }

        $this->load->view('reseller/pre_application/form');
    }
