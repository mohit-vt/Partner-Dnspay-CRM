...
class Tasks extends AdminController
{
    ...
    public function kanban()
    {
  	 ...
  	 public function update_task_assignees_inline($task_id)
	{
    ...
    $success = $this->tasks_model->sync_task_assignees((int)$task_id, $assignees);
   ...
   /* Add new task or update existing */
    public function task($id = '')
    {
        ...
        if ($this->input->post()) {
          ...
          //log_message('error', 'TASK POST HIT: ' . __FILE__ . ' :: ' . __LINE__);
//log_message('error', 'TASK REQUEST URI: ' . ($_SERVER['REQUEST_URI'] ?? ''));
//log_message('error', 'TASK RAW _POST rel: ' . print_r([
    //'rel_type' => $_POST['rel_type'] ?? '(missing)',
    //'rel_id'   => $_POST['rel_id'] ?? '(missing)',
//], true));
//log_message('error', 'TASK CI POST rel: ' . print_r([
    //'rel_type' => $data['rel_type'] ?? '(missing)',
    //'rel_id'   => $data['rel_id'] ?? '(missing)',
//], true));
            ...
            if ($id == '') {
              ...
              // Fallback: if rel fields didn't come in POST but exist in URL (happens when modal opened via relation)
if ((!isset($data['rel_type']) || !$data['rel_type']) && $this->input->get('rel_type')) {
    $data['rel_type'] = $this->input->get('rel_type');
}
if ((!isset($data['rel_id']) || !$data['rel_id']) && $this->input->get('rel_id')) {
