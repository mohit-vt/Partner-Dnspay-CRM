...
public function __construct() 
{
    ...
    // Delete CRM session cookie if exists
    if (isset($_COOKIE['sp_session'])) {
        unset($_COOKIE['sp_session']);
        setcookie('sp_session', '', time() - 3600, '/', '.edata24.com');
    }

    // Destroy CRM session completely
    if ($this->session && method_exists($this->session, 'sess_destroy')) {
        $this->session->sess_destroy();
    }

    // Load affiliate session config ONLY here
    $this->config->load('affiliate_session', TRUE);
    $affiliate_config = $this->config->item('affiliate_session');

    // Start affiliate session
    $this->load->library('session', $affiliate_config);
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            // SUCCESS → $success is now the USER object
             $user = $this->db->get(db_prefix() . 'affiliate_users')->row();
          	
            // Write session ONCE (model already writes, but keep)
            // Store affiliate session data
            $this->session->set_userdata([
                'affiliate_logged_in' => true,
                'affiliate_id'        => $user->id,
                'affiliate_email'     => $user->email,
                'affiliate_ip'        => $this->input->ip_address(),
            ...
            // Check latest session row (temporary debug, remove in prod)
            $latest = $this->db->order_by('timestamp', 'DESC')->get('tblsessions_affiliate', 1)->row();
            log_message('debug', 'AFF SESSION LATEST LEN: ' . (is_object($latest) ? strlen($latest->data) : 'no-row'));
  	...
  	public function force_login($email = 'abi93310@gmail.com')
{
    $this->load->database();
