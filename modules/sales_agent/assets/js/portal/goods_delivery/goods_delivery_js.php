...
"use strict";  
  ...
  init_goods_delivery_currency(<?php echo sa_html_entity_decode($base_currency_id) ?>);
...
$("body").on('change', 'select[name="warehouse_id"]', function() {
  ...
  if(data.commodity_id != '' && data.warehouse_id != ''){
    $.post(site_url + 'sales_agent/portal/get_quantity_inventory', data).done(function(response){
...
function check_quantity_inventory(commodity_id, quantity, warehouse_id, switch_barcode_scanners = false) {
  ...
  if(commodity_id != '' && warehouse_id != '' ){
    $.post(site_url + 'sales_agent/portal/check_quantity_inventory', data).done(function(response){
      response = JSON.parse(response);
...
function wh_add_item_to_preview(id) {
  ...
  requestGetJSON('sales_agent/portal/get_item_by_id_good_receipt/' + id +'/'+true).done(function (response) {
...
function wh_get_item_row_template(name, commodity_name, warehouse_id, available_quantity, quantities, unit_name, unit_price, taxname, lot_number, expiry_date, commodity_code, unit_id, tax_rate, discount, note, guarantee_period, item_key, item_index, formdata, without_checking_warehouse)  {
  ...
  var d = $.post(site_url + 'sales_agent/portal/get_good_delivery_row_template', {
    name: name,
...
function invoice_change(){
  ...
  $.post(admin_url + 'sales_agent/portal/copy_invoices/'+invoice_id).done(function(response){
    response = JSON.parse(response);
      ...
      if (pressed == false) {
        setTimeout(function() {
          if (chars.length >= 8) {
            ...
            requestGetJSON('sales_agent/portal/wh_get_item_by_barcode/' + barcode).done(function (response) {
  ...
  if(parseFloat(data.available_quantity) < parseFloat(data.quantities) && (data.without_checking_warehouse == 0 || data.without_checking_warehouse == '0' ) ){
    ...
    <?php if(get_option('sa_wh_products_by_serial') == 1){ ?>
