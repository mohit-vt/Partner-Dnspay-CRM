    ...
    <div class="panel_s">
       ...
       <div class="panel-body">
          ...
          <div class="row">
           ...
           ?>
        <input type="hidden" name="id" value="<?php echo sa_html_entity_decode($id); ?>">
        <input type="hidden" name="edit_approval" value="<?php echo sa_html_entity_decode($edit_approval); ?>">
        <input type="hidden" name="save_and_send_request" value="false">
        <input type="hidden" name="additional_discount" value="<?php echo sa_html_entity_decode($additional_discount); ?>">
                 ...
                 <div class="form-group">
                  <label for="invoice_id"><?php echo _l('invoices'); ?></label>
                    <select onchange="invoice_change(this); return false;" name="invoice_id" id="invoice_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if($edit_approval == 'true'){ echo 'disabled';} ; ?> >
                        ...
                        <option value="<?php echo sa_html_entity_decode($invoice['id']); ?>" <?php if(isset($goods_delivery) && $goods_delivery->invoice_id == $invoice['id']){ echo 'selected'; } ?>><?php echo get_sa_invoice_number($invoice['id']); ?></option>
                          <?php } ?>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-md-3">
                  ...
                  <label for="customer_code"><?php echo _l('customer_name'); ?></label>
                    <select name="customer_code" id="vendor" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if($edit_approval == 'true'){ echo 'disabled';} ; ?>  >
                        ...
                        <option value="<?php echo sa_html_entity_decode($s['id']); ?>" <?php if(isset($goods_delivery) && $goods_delivery->customer_code == $s['id']){ echo 'selected'; } ?>><?php echo sa_html_entity_decode($s['name']); ?></option>
                          <?php } ?>
                    </select>
                  </div>
              ...
              <div class="form-group">
                  <label for="warehouse_id"><?php echo _l('warehouse_name'); ?></label>
                    <select name="warehouse_id" id="warehouse_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" <?php if($edit_approval == 'true'){ echo 'disabled';} ; ?> >
                        ...
                        <option value="<?php echo sa_html_entity_decode($wh_value['warehouse_id']); ?>" <?php if(isset($goods_delivery) && $goods_delivery->warehouse_id == $wh_value['warehouse_id']){ echo 'selected'; } ?>><?php echo ($wh_value['warehouse_name']); ?></option>
                          <?php } ?>
                    </select>
              </div>
            ...
            <div class="panel-body mtop10 invoice-item">
              ...
              <div class="table-responsive s_table ">
                <table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop">
                  ...
                  <tbody>
                    <?php echo sa_html_entity_decode($goods_delivery_row_template); ?>
                  </tbody>
                </table>
              </div>
