         ...
         <div class="row">
            <div class="col-md-12">
               ...
               <?php if (is_sale_agent_logged_in()) { ?>
                  <a href="<?php echo site_url('sales_agent/portal/purchase_invoices'); ?>" class="btn btn-default pull-right mtop5 mright5 action-button go-to-portal">
         ...
         <div class="row mtop20">
            ...
            <div class="col-sm-6 text-right transaction-html-info-col-right">
               ...
               foreach ($pdf_custom_fields as $field) {
                  ...
                  <p class="no-mbot">
                     <span class="bold"><?php echo sa_html_entity_decode($field['name']); ?>: </span>
                     <?php echo sa_html_entity_decode($value); ?>
                  </p>
               <?php } ?>
            </div>
         </div>
         <div class="row">
            <div class="col-md-12">
               <div class="table-responsive">
                  ...
                  echo sa_html_entity_decode($items->table());
                  ?>
               </div>
            </div>
            ...
            <?php hooks()->do_action('sale_invoice_client_redeem', $invoice); ?>
               ...
               <div class="invoice-html-files">
                  ...
                  ?>
                     <div class="col-md-12 mbot10">
                        ...
                        <a href="<?php echo sa_html_entity_decode($attachment_url); ?>"><?php echo sa_html_entity_decode($attachment['file_name']); ?></a>
                     </div>
                  <?php } ?>
               </div>
            <?php } ?>
            <?php if (!empty($invoice->clientnote)) { ?>
               <div class="col-md-12 invoice-html-note">
                  <b><?php echo _l('invoice_note'); ?></b><br /><br /><?php echo sa_html_entity_decode($invoice->clientnote); ?>
               </div>
            <?php } ?>
            <?php if (!empty($invoice->terms)) { ?>
               <div class="col-md-12 invoice-html-terms-and-conditions">
                  <hr />
                  <b><?php echo _l('terms_and_conditions'); ?>:</b><br /><br /><?php echo sa_html_entity_decode($invoice->terms); ?>
               </div>
            <?php } ?>
            <div class="col-md-12">
               <hr />
                  ...
                  <table class="table table-hover invoice-payments-table">
                     ...
                     <tbody>
                        <?php foreach ($invoice->payments as $payment) { ?>
                           <tr>
                              <td>
                                 <span class="pull-left"><?php echo sa_html_entity_decode($payment['paymentid']); ?></span>
                                 <?php echo form_open($this->uri->uri_string()); ?>
                                 <button type="submit" value="<?php echo sa_html_entity_decode($payment['paymentid']); ?>" class="btn btn-icon btn-default pull-right" name="paymentpdf"><i class="fa fa-file-pdf"></i></button>
                                 <?php echo form_close(); ?>
                              </td>
                              <td><?php echo sa_html_entity_decode($payment['name']); ?> <?php if (!empty($payment['paymentmethod'])) {
                  ...
                  <div class="row">
                        ...
                        <div class="col-md-6 text-left">
                           ...
                           ?>
                                 <div class="radio radio-success online-payment-radio">
                                    <input type="radio" value="<?php echo sa_html_entity_decode($mode['id']); ?>" id="pm_<?php echo sa_html_entity_decode($mode['id']); ?>" name="paymentmode">
                                    <label for="pm_<?php echo sa_html_entity_decode($mode['id']); ?>"><?php echo sa_html_entity_decode($mode['name']); ?></label>
                                 </div>
                                 <?php if (!empty($mode['description'])) { ?>
                                    <div class="mbot15">
                                       <?php echo sa_html_entity_decode($mode['description']); ?>
                                    </div>
                           <?php }
                              }
                           } ?>
                           <div class="form-group mtop25">
                              <?php if (get_option('allow_payment_amount_to_be_modified') == 1) { ?>
                                 ...
                                 <div class="input-group">
                                    <input type="number" required max="<?php echo sa_html_entity_decode($invoice->total_left_to_pay); ?>" data-total="<?php echo sa_html_entity_decode($invoice->total_left_to_pay); ?>" name="amount" class="form-control" value="<?php echo sa_html_entity_decode($invoice->total_left_to_pay); ?>">
                                    <span class="input-group-addon">
                                       <?php echo sa_html_entity_decode($invoice->symbol); ?>
                                    </span>
                                 </div>
                              <?php } else {
                           ...
                           <input type="hidden" name="hash" value="<?php echo sa_html_entity_decode($hash); ?>">
                           <?php echo form_close(); ?>
                        </div>
                     <?php } ?>
                     <?php if (found_invoice_mode($payment_modes, $invoice->id)) { ?>
                        <div class="invoice-html-offline-payments <?php if ($found_online_mode == true) {
                           ...
                           ?>
                                 <p class="bold"><?php echo sa_html_entity_decode($mode['name']); ?></p>
                                 <?php if (!empty($mode['description'])) { ?>
                                    <div class="mbot15">
                                       <?php echo sa_html_entity_decode($mode['description']); ?>
                                    </div>
