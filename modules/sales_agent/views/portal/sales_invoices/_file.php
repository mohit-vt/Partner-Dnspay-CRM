   ...
   <div class="modal-dialog full-screen-modal dialog_withd" role="document">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h4 class="modal-title"><?php echo sa_html_entity_decode($file->file_name); ?></h4>
         </div>
         <div class="modal-body">
            <div class="row">
               <div class="col-md-12 border-right project_file_area">
                  <?php if(!empty($file->external) && $file->external == 'dropbox'){ ?>
                  <a href="<?php echo sa_html_entity_decode($file->external_link); ?>" target="_blank" class="btn btn-info mbot20"><i class="fa fa-dropbox" aria-hidden="true"></i> <?php echo _l('open_in_dropbox'); ?></a><br />
                  <?php } ?>
                  <?php
                     $path = SALES_AGENT_MODULE_UPLOAD_FOLDER.'/sale_invoice/'.$file->rel_id.'/'.$file->file_name;
                     if(is_image($path)){ ?>
                  <img src="<?php echo base_url(SA_PATH.'sale_invoice/'.$file->rel_id.'/'.$file->file_name); ?>" class="img img-responsive ">
                  ...
                  <iframe src="<?php echo base_url(SA_PATH.'sale_invoice/'.$file->rel_id.'/'.$file->file_name); ?>" height="100%" width="100%" frameborder="0"></iframe>
                  <?php } else if(strpos($file->file_name,'.xls') !== false && empty($file->external)){ ?>
                  <iframe src='https://view.officeapps.live.com/op/embed.aspx?src=<?php echo base_url(SA_PATH.'sale_invoice/'.$file->rel_id.'/'.$file->file_name); ?>' width='100%' height='100%' frameborder='0'>
                  </iframe>
                  ...
                  <?php } else if(is_markdown_file($path) && $previewMarkdown = markdown_parse_preview($path)) {
                     echo sa_html_entity_decode($previewMarkdown);
                  } else {
