	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$staffid = '';
				if(isset($member)){
					$title .= _l('pur_update_permissions');
					$staffid    = $member->staffid;
				}else{
					$title .= _l('pur_add_permissions');
				}
				?>
				<h4 class="modal-title"><?php echo sa_html_entity_decode($title); ?></h4>
			</div>
			<?php echo form_open(admin_url('sales_agent/sa_update_permissions/'.$staffid), array('id' => 'update_permissions')); ?>
			<div class="modal-body">
					...
					<div class="hide">
						...
						?>
						<input type="checkbox" name="administrator" id="administrator" <?php echo sa_html_entity_decode($isadmin); ?>>
					</div>
					...
					<?php if(isset($member)){ 
						...
						<div class="lable-display-name">
							<?php echo render_input('staff_name', 'pur_staff_name', get_staff_full_name($member->staffid), '', $staff_attr); ?>
					...
					<div class="class <?php  echo sa_html_entity_decode($display_staff); ?>">
						<?php echo render_select('staff_id',$staffs,array('staffid',array('firstname', 'lastname')),'pur_staff_name',$staff_selected); ?>
					</div>
					...
					<table class="table table-bordered roles no-margin">
						...
						<tbody>
							...
							$hr_profile_permissions = list_sa_permisstion();

							foreach(get_available_staff_permissions($funcData) as $feature => $permission) { ?>
								<tr data-name="<?php echo sa_html_entity_decode($feature); ?>" class="<?php if(!in_array($feature, $hr_profile_permissions)){echo "hide";} ?>">
									<td>
										<b><?php echo sa_html_entity_decode($permission['name']); ?></b>
									</td>
									<td>
										<?php
										if(isset($permission['before'])){
											echo sa_html_entity_decode($permission['before']);
										}
										?>
										...
										<div class="checkbox">
											...
											type="checkbox"
											<?php echo sa_html_entity_decode($checked);?>
											class="capability"
											id="<?php echo sa_html_entity_decode($feature .'_'.$capability); ?>"
											name="permissions[<?php echo sa_html_entity_decode($feature); ?>][]"
											value="<?php echo sa_html_entity_decode($capability); ?>"
											<?php echo sa_html_entity_decode($disabled); ?>>
											<label for="<?php echo sa_html_entity_decode($feature .'_'.$capability); ?>">
									...
									if(isset($permission['after'])){
										echo sa_html_entity_decode($permission['after']);
									}
									?>
								</td>
							</tr>
		...
		<div class="modal-footer">
			<button type="button" class="btn btn-default close_btn" data-dismiss="modal"><?php echo _l('pur_close'); ?></button>
...
<?php require('modules/sales_agent/assets/js/settings/permissions_modal_js.php'); ?>