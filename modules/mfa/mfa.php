...
/**
 * 🔒 Mandatory MFA Enforcement Hooks
 * Force all staff to enable and verify MFA before accessing admin area
 */
hooks()->add_action('after_staff_login', function ($staff) {
    $CI = &get_instance();
    $staffid = $staff->staffid;
    $CI->load->model('staff_model');
    $staffData = $CI->staff_model->get($staffid);

    // If MFA not enabled for this staff → go to setup
    if ($staffData->mfa_google_ath_enable != 1 || empty($staffData->gg_auth_secret_key)) {
        redirect(admin_url('mfa/mfa_auth/multi_factor_authentication/' . $staffid));
    }

    // If enabled but not yet verified in this session → go to OTP verification
...
hooks()->add_action('admin_auth_init', function () {
    ...
    // Avoid redirect loop for MFA controller itself
    if ($CI->router->fetch_class() === 'mfa_auth') {
        return;
    }
...
function mfa_module_init_menu_items() {
	...
	$CI->app_menu->add_sidebar_menu_item('mfa', [
		'name' => _l('mfa'),
		'icon' => 'fa fa-lock',
		'position' => 20,
	]);

	$CI->app_menu->add_sidebar_children_item('mfa', [
        'slug'     => 'mfa-management',
