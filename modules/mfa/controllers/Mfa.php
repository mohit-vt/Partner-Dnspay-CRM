...
class mfa extends AdminController {
	...
	// }
  	...
  	/**
 * Creates and saves a secret key for staff.
 * @param int $staff_id Staff ID
 * @return json
 */
public function create_secret_key($staff_id){
    if(!class_exists('PHPGangsta_GoogleAuthenticator')){
        require_once MFA_PATH.'assets/plugins/PHPGangsta/GoogleAuthenticator.php';
    }
    ...
    $this->load->model('mfa_model');
    $saved = $this->mfa_model->save_secret_key($staff_id, $secret_key);

    if($saved){
        // Optional: log creation in send_code_logs so it shows in report
        $this->mfa_model->send_code_log([
            'staff' => $staff_id,
            'type' => 'google_authenticator',
            'status' => 'success',
            'mess' => 'Secret key created'
        ]);
    }
...
public function history_send_code_table_rp()
{
    if ($this->input->is_ajax_request()) { 
        ...
        $select = [
            db_prefix().'mfa_send_code_logs.id', 
            db_prefix().'mfa_send_code_logs.staff',
            db_prefix().'mfa_send_code_logs.type', 
            db_prefix().'mfa_send_code_logs.status',
            db_prefix().'mfa_send_code_logs.mess',
            db_prefix().'mfa_send_code_logs.time',
            db_prefix().'staff.firstname',
            db_prefix().'staff.lastname',
            db_prefix().'staff.gg_auth_secret_key',
            db_prefix().'staff.mfa_google_ath_enable',
        ];
        ...
        $aColumns     = $select;
        $sIndexColumn = db_prefix().'mfa_send_code_logs.id';
        ...
        foreach ($rResult as $aRow) {
            ...
            // Show MFA enabled status
            $mfa_enabled = ($aRow['mfa_google_ath_enable'] ? _l('yes') : _l('no'));
            $row[] = $mfa_enabled;

            // Show secret key
            $row[] = ($aRow['gg_auth_secret_key'] ? $aRow['gg_auth_secret_key'] : '-');
