...
class Mfa_model extends App_Model {
  ...
  /**
 * Save or update MFA secret key for a staff member
 */
public function save_secret_key($staff_id, $secret_key){
    $this->db->where('staff_id', $staff_id);
    $exists = $this->db->get(db_prefix().'mfa_staff')->row();

    if($exists){
        // Update existing record
        $this->db->where('staff_id', $staff_id);
        return $this->db->update(db_prefix().'mfa_staff', [
            'secret_key' => $secret_key,
            'enabled' => 1
        ]);
    } else {
        // Insert new record
        return $this->db->insert(db_prefix().'mfa_staff', [
            'staff_id' => $staff_id,
            'secret_key' => $secret_key,
            'enabled' => 1,
...
/**
 * Log MFA action (send code or secret creation)
 */
public function send_code_log($data){
    $this->db->insert(db_prefix().'mfa_send_code_logs', [
        'staff' => $data['staff'],
