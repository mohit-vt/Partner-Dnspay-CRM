...
/*
Module Name: Landing Page builder
...
hooks()->add_action('admin_init', 'zillapage_module_init');

function zillapage_module_init()
{
    $CI = &get_instance();
    $module = $CI->db->get_where('tblmodules', ['module_name' => 'zillapage'])->row();
    
    if (!$module) {
        // If not found, insert it manually
        $CI->db->insert('tblmodules', [
            'module_name' => 'zillapage',
            'installed'   => 1,
            'version'     => '1.0.6'
        ]);
    }
...
if (!function_exists('module_zillapage_activate')) {
    function module_zillapage_activate()
    {
        $CI = &get_instance();
        require_once(__DIR__ . '/install.php'); // Ensure installation runs

        if (!$CI->db->field_exists('module_name', 'tblmodules')) {
            $CI->db->query("ALTER TABLE `tblmodules` ADD `module_name` VARCHAR(255) NOT NULL;");
        }

        $CI->db->query("INSERT IGNORE INTO `tblmodules` (`module_name`,`active`, `installed_version`)
                        VALUES ('zillapage', 1, '1.0.6');");
    }
}

// Run activation function on module load
module_zillapage_activate();
