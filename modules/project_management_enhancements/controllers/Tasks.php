...
class Tasks extends AdminController {
    public function __construct()
    {
        ...
        $this->ensure_module_active(); // Ensure the module is always active
    }
	...
	private function ensure_module_active()
	{
		...
		if ($module && $module->active == 0) {
			// Activate the module if not already active
			$this->db->where('module_name', 'project_management_enhancements')
				->update('modules', ['active' => 1]);
		}
	
		// Optional: Skip activation checks in development
    ...
    /* Open all tasks if user accesses this /tasks URL */
    ...
    {
        App_table::find('tasks_enhance')->output();
    }
    ...
    public function ajax_search_assign_task_to_timer()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->db->where(
                db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix(
                ) . 'task_assigned WHERE staffid = ' . get_staff_user_id() . ')'
            );
	...
	public function detailed_overview()
	{
		...
		for ($m = 1; $m <= 12; $m++)
		{
			...
			if (is_numeric($staff_id))
			{
				$sqlTasksSelect .= ',(SELECT COUNT(id) FROM '.db_prefix(
					).'task_checklist_items WHERE taskid='.db_prefix(
					).'tasks.id AND finished=1 AND finished_from='.$staff_id.') as total_finished_checklist_items';
			} else
			{
			...
			$selectTotalComments = ',(SELECT COUNT(id) FROM '.db_prefix().'task_comments WHERE taskid='.db_prefix(
				).'tasks.id';
			$selectTotalFiles = ',(SELECT COUNT(id) FROM '.db_prefix().'files WHERE rel_id='.db_prefix(
				).'tasks.id AND rel_type="task"';
			...
			// Task assignees
			$sqlTasksSelect .= ','.get_sql_select_task_asignees_full_names(
				).' as assignees'.','.get_sql_select_task_assignees_ids().' as assignees_ids';
			...
			} elseif ($has_permission_view)
			{
				...
				{
					$this->db->where(
						'(id IN (SELECT taskid FROM '.db_prefix().'task_assigned WHERE staffid='.$staff_id.'))'
					);
				}
			}
		...
		$data['years'] = $this->tasks_model->get_distinct_tasks_years(
			($this->input->post('month_from') ? $this->input->post('month_from') : 'startdate')
		);
...
// --- Option A: fallback from hidden fields (and then unset them) ---
$rt_hidden  = $this->input->post('rel_type_hidden');
$rid_hidden = $this->input->post('rel_id_hidden');

if ((empty($data['rel_type']) || empty($data['rel_id'])) && !empty($rt_hidden) && !empty($rid_hidden)) {
    $data['rel_type'] = $rt_hidden;
    $data['rel_id']   = $rid_hidden;
}

// IMPORTANT: do not let these go to the model insert
unset($data['rel_type_hidden'], $data['rel_id_hidden']);
		...
		if ($this->input->get('rel_type') == 'project' && $this->input->get(
				'rel_id'
			) || ($id !== '' && $data['task']->rel_type == 'project'))
		{
			...
			$data['task_types'] = $this->task_types_model->get_project_task_types($this->input->get('rel_id'));
		}
		...
		if ( ! isset($data['task_types'])
			&& isset($data['task'])
			&& isset($data['task']->rel_id)
		)
		{
			$data['task_types'] = $this->task_types_model->get_project_task_types($data['task']->rel_id);
		} elseif (isset($id) && $data['task'])
		{
			$data['task_types'] = $this->task_types_model->get_project_task_types($data['task']->rel_id);
		} elseif ( ! isset($data['task_types']))
		{
			$data['task_types'] = [];
		}
	...
	public function get_task_data($taskid, $return = FALSE)
	{
		...
		if ($task->task_type)
		{
			$data['task_type'] = $this->task_types_model->get_task_type_by_task_type_id(
				$task->rel_id,
				$task->task_type
			);
		} else
		...
		if ($return == FALSE)
		{
			$this->load->view('tasks/view_task_template', $data);
		} else
		{
			return $this->load->view('tasks/view_task_template', $data, TRUE);
		}
	}
	...
	public function init_checklist_items()
	{
		if ($this->input->is_ajax_request())
		{
			if ($this->input->post())
			{
				...
				$data['task_staff_members'] = $this->tasks_model->get_staff_members_that_can_access_task(
					$data['task_id']
				);
				$data['current_user_is_creator'] = $this->tasks_model->is_task_creator(
					get_staff_user_id(),
					$data['task_id']
				);
				$data['hide_completed_items'] = get_staff_meta(
					get_staff_user_id(),
					'task-hide-completed-items-'.$data['task_id']
				);
	...
	public function update_checklist_item()
	{
		if ($this->input->is_ajax_request())
		{
			if ($this->input->post())
			{
				...
				echo json_encode(
					[
						'can_be_template' => (total_rows(
								db_prefix().'tasks_checklist_templates',
								['description' => $desc]
							) == 0),
					]
				);
	...
	public function add_task_comment()
	{
		...
		if (
			$data['content'] != ''
			|| (isset($_FILES['file']['name']) && is_array($_FILES['file']['name']) && count(
					$_FILES['file']['name']
				) > 0)
		)
		{
			...
			if ($comment_id)
			{
				...
				if ($commentAttachments && is_array($commentAttachments))
				{
					...
					{
						$this->db->query(
							'UPDATE '.db_prefix()."task_comments SET content = CONCAT(content, '[task_attachment]')
	...
	public function upload_file()
	{
		if ($this->input->post())
		{
			...
			if ($files)
			{
				...
				foreach ($files as $file)
				{
					$success = $this->tasks_model->add_attachment_to_database(
						$taskid,
						[$file],
						FALSE,
						($i == $len - 1 ? TRUE : FALSE)
	...
	{
		if (staff_can('delete_timesheet', 'tasks') || staff_can('delete_own_timesheet', 'tasks') && total_rows(
				db_prefix().'taskstimers',
				['staff_id' => get_staff_user_id(), 'id' => $id]
			) > 0)
		{
			$alert_type = 'warning';
	...
	public function update_timesheet()
	{
		if ($this->input->is_ajax_request())
		{
			if (staff_can('edit_timesheet', 'tasks') || (staff_can('edit_own_timesheet', 'tasks') && total_rows(
						db_prefix().'taskstimers',
						['staff_id' => get_staff_user_id(), 'id' => $this->input->post('timer_id')]
					) > 0))
			{
								...
								if ( ! $this->tasks_model->is_task_assignee($user_id, $id))
								{
									...
									if ($task->rel_type == 'project')
									{
										...
										if (total_rows(
												...
												['project_id' => $task->rel_id, 'staff_id' => $user_id]
											) == 0)
										{
											$this->db->insert(
												db_prefix().'project_members',
