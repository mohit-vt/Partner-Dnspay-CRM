...
<?php echo form_open_multipart(admin_url('project_management_enhancements/tasks/task' . ($id ? '/' . $id : '')), ['id' => 'task-form']); ?>
            ...
            <div class="modal-body">
                <div class="row">
                  ...
                  ?>
                      ...
                      <input type="hidden" name="rel_type_hidden" id="rel_type_hidden" value="<?php echo html_escape($rel_type); ?>">
<input type="hidden" name="rel_id_hidden" id="rel_id_hidden" value="<?php echo html_escape($rel_id); ?>">
                   ...
                   echo ' hide';
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="task_type"
                                           class="control-label"><?php echo _l('task_add_edit_task_type'); ?></label>
                                    <select name="task_type" class="selectpicker" id="task_type" data-width="100%"
                                            data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
										<?php foreach ($task_types as $task_type) { ?>
                                            <option style="color: <?php echo $task_type['label_color'] ?>" value="<?php echo e($task_type['id']); ?>" <?php if (isset($task) && $task->task_type == $task_type['id']) {echo ' selected';} ?>><?php echo e($task_type['name']); ?></option>
										<?php } ?>
                                    </select>
                                </div>
                            </div>
    ...
    $(function() {
      ...
      // ---- Keep hidden relation values stable for submit (Option A) ----

// set initial values (when modal opens)
// Keep server-provided hidden rel values (do not overwrite with empty ajax values)
var rt_hidden = $('#_task_modal #rel_type_hidden').val();
var rid_hidden = $('#_task_modal #rel_id_hidden').val();

// If rel_type select is empty but hidden has value, restore it into the selectpicker
if (rt_hidden && !_rel_type.val()) {
  _rel_type.val(rt_hidden);
  _rel_type.selectpicker('refresh');

  // ensure rel_id ajax + UI are initialized correctly
  _rel_id_wrapper.removeClass('hide');
  task_rel_select();
}


// If rel_id is empty but hidden has value, keep hidden as source of truth.
// (rel_id select is ajax; it may not have a value at init time)


// when rel_type changes
$('body').on('change', '#_task_modal #rel_type', function () {
  $('#_task_modal #rel_type_hidden').val($(this).val() || '');
});

// when rel_id changes (this one is re-created dynamically, so use body delegation)
$('body').on('change', '#_task_modal #rel_id', function () {
  $('#_task_modal #rel_id_hidden').val($(this).val() || '');
});
...
function task_rel_select() {
  ...
  // Prefer hidden value (source of truth when modal opens from project)
  serverData.rel_id = $('#_task_modal #rel_id_hidden').val() || _rel_id.val();

  data.type = $('#_task_modal #rel_type_hidden').val() || _rel_type.val();

  init_ajax_search(data.type, _rel_id, serverData);
}
