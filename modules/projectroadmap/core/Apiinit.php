...
namespace modules\projectroadmap\core;

require_once __DIR__.'/../third_party/node.php';
require_once __DIR__.'/../vendor/autoload.php';
use Firebase\JWT\JWT as Projectroadmap_JWT;
use Firebase\JWT\Key as Projectroadmap_Key;
use WpOrg\Requests\Requests as Projectroadmap_Requests;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =Projectroadmap_JWT::decode($token, new Projectroadmap_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = Projectroadmap_Requests::post(VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
    ...
    {
        get_instance()->load->library('projectroadmap_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->projectroadmap_aeiou->getPurchaseData($code);
        ...
        try {
            ...
            $response = Projectroadmap_Requests::post(REG_PROD_POINT, ['Accept' => 'application/json'], $data);
