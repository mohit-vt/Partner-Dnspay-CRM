...
namespace modules\logtracker\core;
...
class Apiinit
{
    public static function the_da_vinci_code($module_name)
    {
        ...
        if (4 === \count($id_data)) {
            ...
            try {
                $data =logtracker_JWT::decode($token, new logtracker_Key($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                $verified = false;
                try {
                    $request = logtracker_Requests::post(CTL_VAL_PROD_POINT, ['Accept' => 'application/json', 'Authorization' => $token], json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url(), 'version' => $module['headers']['version']]));
                    $status  = $request->status_code;
                    if ((500 <= $status && $status <= 599) || 404 == $status) {
                        update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $status, 'id' => $token, 'end_point' => CTL_VAL_PROD_POINT])));
                        $verified = false;
                    } else {
    ...
    {
        get_instance()->load->library('logtracker_aeiou');
        $module = get_instance()->app_modules->get($module_name);
        if (empty($code)) {
        ...
        $envato_res = get_instance()->logtracker_aeiou->getPurchaseData($code);
        ...
        try {
            $response = logtracker_Requests::post(CTL_REG_PROD_POINT, ['Accept' => 'application/json'], $data);

            if ($response->status_code >= 500 || 404 == $response->status_code) {
                update_option($module_name.'_verification_id', '');
                update_option($module_name.'_last_verification', time());
                update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $response->status_code, 'id' => $code, 'end_point' => CTL_REG_PROD_POINT])));
            ...
            update_option($module_name.'_verification_id', '');
            update_option($module_name.'_last_verification', time());
            update_option($module_name.'_heartbeat', base64_encode(json_encode(['status' => $request->status_code, 'id' => $code, 'end_point' => CTL_REG_PROD_POINT])));
