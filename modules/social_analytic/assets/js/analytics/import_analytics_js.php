<script>
   (function($) {
    "use strict";
      ...
      {
        $( "#dowload_file_sample" ).append( '<a href="'+ site_url+'modules/social_analytic/uploads/file_sample/Sample_import_analytics_file_vi.xlsx?v=10" class="btn btn-primary" ><?php echo _l('download_sample') ?></a><hr>' );

      }else{
        $( "#dowload_file_sample" ).append( '<a href="'+ site_url+'modules/social_analytic/uploads/file_sample/Sample_import_analytics_file_en.xlsx?v=10" class="btn btn-primary" ><?php echo _l('download_sample') ?></a><hr>' );
      }
...
function uploadfilecsv(){
  "use strict";
    ...
    if($('select[name="account_id"]').val() == ''){
      alert_float('warning', "<?php echo _l('please_select_a_account') ?>");
      
      return false;
    }
    var formData = new FormData();
    formData.append("file_csv", $('#file_csv')[0].files[0]);
    if(<?php echo  sa_check_csrf_protection(); ?>){
        formData.append(csrfData.token_name, csrfData.hash);
    }

    formData.append("leads_import", $('input[name="leads_import"]').val());
    formData.append("account_id", $('select[name="account_id"]').val());
    ...
    $.ajax({ 
      url: admin_url + 'social_analytic/import_file_xlsx_analytics', 
      method: 'post', 
      data: formData, 
      contentType: false, 
      processData: false
