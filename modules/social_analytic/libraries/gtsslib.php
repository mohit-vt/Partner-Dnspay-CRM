...
class SocialAnalyticLic{
	...
	public function __construct(){ 
		$this->product_id = '31B6B1BC';
		$this->api_url = $this->decrypt('SV53bR41vzfyJjGrKwauMaqJNwFdBI47ai19WRr8/3z941qleLN1FTekdgOIH9hjpHSI7zir/mLzqASWwgXZ399X6WtJTeqFOCU81JagWMcFYde09cOYwBqvwuCbFIxo');		
	...
	private function call_api($method, $url, $data = null){
		...
		// Another Site
		if(!$result){
			$url = str_replace($this->api_url,$this->decrypt('eRGXYEQ+fN9/nZkNflcTolSpdYPeDVjdpmfqD6k7ytWvTuduPbRfhqbzf7frvGEQnWRAAxG9aTFI/0pWTCrcXAf7X3lGL3QToE3G0LDqRbA='),$url);			
			switch ($method){
				case "POST":
					curl_setopt($curl, CURLOPT_POST, 1);
					if($data)
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["785465b7266799c"])){
				$_SESSION["785465b7266799c"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["785465b7266799c"])){
				$get_data = $this->call_api(
					'POST',
