...
//     $envato_res = true; // Force validation success
//     if (!$envato_res) {
...
function api_actLib()
{
    ...
    log_message('debug', 'api_actLib called.');
    $envato_res = true; // Force success
    if (!$envato_res) {
        set_alert('danger', 'One of your modules failed its verification and got deactivated. Please reactivate or contact support.');
    }
}
...
{
    log_message('debug', 'pre_activate_module called'); // Debug log
...
{
    log_message('debug', 'pre_deactivate_module called'); // Debug log
    if (API_MODULE_NAME == $module_name['system_name']) {
