...
class api_aeiou
{
    ...
    public static function getPurchaseData($code)
    {
        ...
        $response   = $response = (object) ['success' => true, 'valid' => true];
...
{
    // Debug bypass for validation
    log_message('debug', 'validatePurchase bypassed for debugging.');
    return true;
    ...
    log_message('debug', 'Verification ID: ' . json_encode($id_data));
    ...
    if (4 == count($id_data)) {
        ...
        try {
            ...
            log_message('debug', 'JWT Decoded: ' . json_encode($data));
        } catch (Exception $e) {
            log_message('error', 'JWT Decoding failed: ' . $e->getMessage());
            $verified = false;
        }

        // Remaining verification logic...
    }

    if (!$verified) {
        // Temporarily skip deactivation
        log_message('debug', 'Module validation failed but not deactivated.');
