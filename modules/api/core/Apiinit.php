...
{
    $module = get_instance()->app_modules->get($module_name);
    $verification_id = !empty(get_option($module_name . '_verification_id')) 
        ? base64_decode(get_option($module_name . '_verification_id')) 
        : '';
    $token = get_option($module_name . '_product_token');

    log_message('debug', 'Verification ID: ' . $verification_id);
    log_message('debug', 'Product Token: ' . $token);
    ...
    if ($verified) {
        try {
            $data = api_JWT::decode($token, new api_Key($id_data[3], 'HS512'));
            log_message('debug', 'Decoded JWT Data: ' . json_encode($data));
            $verified = basename($module['headers']['uri']) == $data->item_id 
    ...
    if (!$verified) {
        log_message('debug', 'Module verification failed, but deactivation is bypassed.');
