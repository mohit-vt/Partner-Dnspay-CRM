...
//     if (!is_admin()) {
//         access_denied('Logout All Users');
//     }
...
//     // Delete all sessions except current admin
//     if ($driver === 'database') {
//         $this->db->where('id !=', $current_session_id);
//         $this->db->delete($save_path);

//     } elseif ($driver === 'files') {
//         foreach (glob(rtrim($save_path, '/').'/*') as $file) {
//             if (is_file($file) && basename($file) !== $current_session_id) {
//                 @unlink($file);
//             }
//         }
//     }

//     set_alert('success', 'All other users have been logged out successfully.');
...
public function logout_all_users()
{
    $super_admin_email = 'mohit.tv@edata24.com';

    $logged_in_staff_id = get_staff_user_id();
    $logged_in_email    = get_staff($logged_in_staff_id)->email ?? null;

    if ($logged_in_email !== $super_admin_email) {
        access_denied('Kill Switch Access Restricted');
    }
    ...
    // Ensure proper session table name
    if ($driver === 'database') {
        if (empty($save_path)) {
            $save_path = db_prefix() . 'sessions';
        } elseif (strpos($save_path, db_prefix()) === false) {
            $save_path = db_prefix() . $save_path;
        }
    }

    $active_user_ids = [];
    $super_admin_session_ids = [];

    if ($driver === 'database') {
        $sessions = $this->db->get($save_path)->result();
        ...
        foreach ($sessions as $session) {
            ...
            // Extract staff_user_id
            if (preg_match('/staff_user_id\|s:\d+:"(\d+)";/', $data, $matches)) {
                $staff_id = (int)$matches[1];
                $email = get_staff($staff_id)->email ?? null;

                if ($email === $super_admin_email) {
                    $super_admin_session_ids[] = $session->id;
                } else {
                    $active_user_ids[] = $staff_id;
                }
            }
        }
    }

    $active_user_ids = array_unique(array_filter($active_user_ids));

    // Disable active users except Super Admin
    if (!empty($active_user_ids)) {
        $this->db->where_in('staffid', $active_user_ids);
        $this->db->where('email !=', $super_admin_email);
        $this->db->update(db_prefix() . 'staff', ['active' => 0]);
    }

    // Delete all sessions except current and super admin's
    if ($driver === 'database') {
        $this->db->where('id !=', $session_id);

        if (!empty($super_admin_session_ids)) {
            $this->db->where_not_in('id', $super_admin_session_ids);
        }

        $this->db->delete($save_path);
    } elseif ($driver === 'files' && is_dir($save_path)) {
        foreach (glob(rtrim($save_path, '/') . '/*') as $file) {
            $sid = basename($file);
            if (is_file($file) && $sid !== $session_id && !in_array($sid, $super_admin_session_ids)) {
                @unlink($file);
            }
        }
    }

    // Record the global force logout timestamp
    update_option('force_logout_timestamp', time());

    log_activity("KILL SWITCH executed by {$super_admin_email}. " . count($active_user_ids) . " users disabled. Super Admin session preserved.");
    set_alert('success', 'Kill Switch executed successfully. All other users were logged out and disabled, Super Admin session preserved.');

    // Preserve current login context before redirect
    $context = $this->session->userdata('login_context') ?? 'admin';
    $this->session->set_flashdata('force_logout_context', $context);

  if ($context === 'reseller') {
    redirect(site_url('reseller/authentication/force_logout_notice'));
} else {
