...
class Facebookleadsintegration_aeiou
{
    ...
    {
        $givemecode = Facebookleadsintegration_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Facebookleadsintegration_Requests::get($verify_url.'?code='.$code, $headers, $options);
        ...
        if (4 == count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Facebookleadsintegration_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
