...
function quickbooks_deprecated_hook_trigger_error(){
    ...
    // $qi_api = new QuickbooksLic();
    // $qi_gtssres = $qi_api->verify_license(true);    
    // if(!$qi_gtssres || ($qi_gtssres && isset($qi_gtssres['status']) && !$qi_gtssres['status'])){
    //      $CI->app_modules->deactivate(QUICKBOOKS_INTEGRATION_MODULE_NAME);
...
function quickbooks_integration_preactivate($module_name){
    ...
    //     require_once 'libraries/gtsslib.php';
    //     $qi_api = new QuickbooksLic();
    //     $qi_gtssres = $qi_api->verify_license();          
    //     if(!$qi_gtssres || ($qi_gtssres && isset($qi_gtssres['status']) && !$qi_gtssres['status'])){
    //          $CI = & get_instance();
    //         $data['submit_url'] = $module_name['system_name'].'/gtsverify/activate'; 
    //         $data['original_url'] = admin_url('modules/activate/'.QUICKBOOKS_INTEGRATION_MODULE_NAME); 
    //         $data['module_name'] = QUICKBOOKS_INTEGRATION_MODULE_NAME; 
